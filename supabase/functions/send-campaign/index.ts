  ...
  try {
    ...
    if (!profile || profile.credits < requiredCoins) {
      return new Response(JSON.stringify({
        error: `Créditos insuficientes. Você precisa de ${requiredCoins} coins (tem ${profile?.credits ?? 0}).`,
        required: requiredCoins, have: profile?.credits ?? 0,
      }), { status: 402, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    // Debita coins na hora (estilo Meta Ads — você paga pelo alcance contratado)
